...
%       SoC(t)
    ...
    while max_SoC - min_SoC < soctimewindow and iter_end < time-1 and iter_init < iter_end:
    ...
    while iter_end < time-1 :
        ...
        while max_SoC - min_SoC < moving_step and iter_end < time - 1 and iter_init < iter_end:
            actual_SoC = actual_SoC - (1 / battery_capacity) * i[iter_end] * dt
            iter_end = iter_end + 1
            max_SoC = max(max_SoC, actual_SoC)
            min_SoC = min(min_SoC, actual_SoC)

        #print( "the shape is",vocv_est[iter_init:iter_end+1].shape)
