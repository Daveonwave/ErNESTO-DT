...
class BaseSimulator:
    ...
    def get_instance(cls, mode: str):
        ...
        match = next((c for c in cls.__subclasses__() if mode in c.__name__.lower()), None)
        if match is None:
            raise StopIteration(f"No subclass of {cls.__name__} found with '{mode}' in its name.")
        return match
